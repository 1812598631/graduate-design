...
void Oled_write_data_1(unsigned char x,unsigned char y,unsigned int data1)
{
   unsigned char translateData[2];
    translateData[0] = data1 % 10 + '0';					
		OLED_ShowStr(x,y,translateData,1) ;
	/*
	for(i=0;i<6;i++)
	printf("%c",translateData[i]);
	*/
    
}
void Oled_write_data_3(unsigned char x,unsigned char y,int data1)
{
	 unsigned int temp[6];
//	 int i;
   unsigned char translateData[6];
	...
	if(data1<0)
	{
		data1=-data1;
	    temp[0] ='-';					//取百位
		translateData[0]=temp[0];
    temp[1] = data1/100%10 + '0';		//取百位
	  translateData[1]=temp[1];
	  temp[2] = data1/10%10 + '0';				//取十位
	  translateData[2] = temp[2];
	  translateData[3] = '.';
    translateData[4] = data1 % 10 + '0';	//取小数点
    translateData[5] = '\0';
OLED_ShowStr(x,y,translateData,1) ;
	}
	else
